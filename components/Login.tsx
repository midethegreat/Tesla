      ...
      <div className="w-full max-w-[700px] glass-card p-6 md:p-16 rounded-[2rem] border border-white/10 z-10 text-center shadow-2xl animate-in fade-in zoom-in duration-500">
        ...
        <form onSubmit={handleLogin} className="space-y-6 md:space-y-8 text-left max-w-sm mx-auto">
          ...
          <div className="pt-4 flex flex-col items-center space-y-6 md:space-y-8">
            <button disabled={loading} type="submit" className="w-full py-4 md:py-5 rounded-full bg-gradient-to-r from-orange-400 to-red-500 text-white font-black uppercase text-[10px] tracking-widest shadow-2xl hover:scale-105 transition active:scale-95 flex items-center justify-center gap-3">
              {loading ? <Loader2 className="animate-spin" size={16} /> : "Account Login"}
            </button>
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Don't have an account? <span onClick={onRegisterClick} className="text-white hover:underline cursor-pointer">Sign Up</span></p>
          </div>
        </form>
      </div>
    </div>
