...
const BadgeCard: React.FC<{ title: string; condition: string }> = ({ title, condition }) => (
  <div className="w-full bg-[#1c1c1c]/40 border border-white/5 rounded-[2.5rem] p-10 md:p-12 flex flex-col items-center justify-center text-center space-y-8 animate-in fade-in zoom-in duration-500 hover:border-white/10 transition-colors group">
    <div className="space-y-1">
      ...
      <span className="text-[10px] md:text-[12px] font-medium text-gray-500 uppercase tracking-[0.3em] block">badge</span>
    </div>
  ...
  <div className={`relative w-full rounded-[1.5rem] overflow-hidden border transition-all duration-500 flex flex-col glass-card group ${plan.isHot ? 'border-amber-500/30 bg-[#1a140d]/80' : 'border-white/5 bg-[#0d0d0d]/80'}`}>
    <div className="relative h-28 flex flex-col justify-center px-6 overflow-hidden flex-shrink-0">
      ...
      <div className="flex items-center gap-2 mt-1 relative z-10">
        ...
        {plan.isHot && <span className="bg-orange-500/20 text-orange-400 text-[8px] font-black px-2 py-0.5 rounded border border-orange-500/20 uppercase tracking-widest">HOT SCHEMA</span>}
      </div>
    </div>

    <div className="px-6 py-6 space-y-4 flex-grow flex flex-col">
      {[
        { label: 'Investment', val: `$${plan.minInvestment}-$${plan.maxInvestment}`, icon: <MousePointer2 size={12} className="text-gray-500" /> },
        { label: 'Capital Back', val: plan.capitalBack ? 'Yes' : 'No', icon: <Briefcase size={12} className="text-gray-500" /> },
      ...
      <div className="pt-6 mt-auto flex flex-col items-center gap-3">
        ...
        <p className="text-center text-[8px] text-amber-500/60 font-black uppercase tracking-[0.2em]">* No Profit Holidays</p>
      </div>
    </div>
  </div>
);
...
const Dashboard: React.FC<DashboardProps> = ({ onLogout }) => {
  ...
  const registrationDate = auth.currentUser?.metadata.creationTime ? new Date(auth.currentUser.metadata.creationTime).toLocaleDateString() : 'N/A';
  ...
  const referralId = auth.currentUser?.uid.slice(0, 8) || 'YbYSaqsQ';
  ...
  const handleTabChange = (id: TabType) => {
    ...
    if (!isKycVerified && (now - lastKycWarningTimeRef.current > cooldownPeriod)) {
      setKycWarningVisible(false);
      setTimeout(() => {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>, target: 'avatar' | 'front' | 'back' | 'selfie') => {
  ...
  const stats = [
    ...
    { label: "Total Withdraw", value: "$0.00", color: "bg-[#eaf4ff]", textColor: "text-black", icon: <ArrowUpCircle size={16} /> },
         ...
         <div className="lg:col-span-7">
            <div className="glass-card bg-[#141414] border border-white/5 rounded-[2.5rem] p-1 overflow-hidden shadow-2xl h-full flex flex-col">
              <div className="bg-gradient-to-br from-[#ffd700] via-[#ffcc00] to-[#ffa500] p-6 md:p-10 rounded-[2.3rem] space-y-4 flex-grow relative overflow-hidden">
                ...
                <span className="text-black font-black text-4xl md:text-6xl block tracking-tighter">${availableBalance}</span>
                <div className="flex items-center gap-2 text-black/60">
                  <Wallet size={18} />
                  <span className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.2em]">Available Balance</span>
                </div>
              </div>
              <div className="p-4 md:p-6 flex flex-col md:flex-row gap-4">
                <button onClick={() => handleTabChange('add-money')} className="flex-grow py-4 rounded-full bg-gradient-to-r from-orange-500 to-red-600 text-white font-black uppercase text-[11px] tracking-widest shadow-2xl shadow-orange-500/30 hover:scale-[1.02] transition transform active:scale-95">Deposit Now</button>
                <button onClick={() => handleTabChange('all-schemas')} className="flex-grow py-4 rounded-full bg-white/5 text-gray-400 font-black uppercase text-[11px] tracking-widest border border-white/10 flex items-center justify-center gap-3 hover:bg-white/10 transition"><ArrowUpRight size={16} /> Invest</button>
              </div>
            </div>
         </div>
         <div className="lg:col-span-5 flex flex-col gap-6">
            <div className="bg-[#141414] border border-white/5 rounded-3xl p-6 flex items-center justify-between group hover:border-amber-500/20 transition">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl bg-amber-500/10 flex items-center justify-center text-amber-500 border border-amber-500/20 group-hover:scale-110 transition"><Award size={24} /></div>
            ...
            <div className="bg-[#141414] border border-white/5 rounded-3xl p-6 space-y-4">
               <div className="flex justify-between items-center"><h4 className="text-[10px] font-black uppercase tracking-widest text-gray-400">Invite Friends</h4><div className="text-[10px] font-bold text-amber-500 uppercase tracking-widest">Share & Earn</div></div>
               <div className="flex items-center gap-3 bg-black/40 rounded-2xl px-4 py-4 md:px-5 border border-white/5 hover:border-white/20 transition group overflow-hidden">
                  ...
                  <span className="text-[10px] md:text-[11px] font-bold text-gray-500 truncate flex-grow">{referralUrl}</span>
                  <button onClick={handleCopy} className="text-gray-400 hover:text-white transition p-1 flex-shrink-0">{copied ? <Check size={16} className="text-green-500" /> : <CopyIcon size={16} />}</button>
               </div>
            </div>
            <div className="grid grid-cols-3 gap-4 flex-grow">
               {[
                 ...
                 <button key={i} onClick={() => handleTabChange(btn.id as TabType)} className="flex flex-col items-center justify-center gap-2 p-3 md:p-4 rounded-3xl bg-[#141414] border border-white/5 hover:border-amber-500/30 hover:bg-amber-500/5 transition-all group">
                    <div className="text-gray-600 group-hover:text-amber-500 transition-transform group-hover:scale-110 duration-300">{btn.icon}</div>
                    <span className="text-[8px] md:text-[9px] font-black uppercase tracking-widest text-gray-500 group-hover:text-white transition text-center">{btn.name}</span>
                 </button>
               ))}
            </div>
         </div>
        ...
        <div className="bg-[#1a1a1a] border border-white/5 rounded-[2.5rem] p-6 md:p-8 shadow-2xl relative">
          ...
          <div className="grid grid-cols-3 gap-y-10 gap-x-2">
            {[
              ...
              { id: 'logs', name: 'Invest Logs', icon: <div className="relative"><FolderOpen size={22} /><div className="absolute top-0 right-0 w-2 h-2 bg-white rounded-full border-2 border-[#1a1a1a]"></div></div> },
              { id: 'transactions', name: 'Transactions', icon: <div className="flex flex-col -space-y-1.5"><ChevronRight size={14} className="rotate-0 ml-2" /><ChevronLeft size={14} className="rotate-0" /></div> },
              { id: 'add-money', name: 'Deposit', icon: <DollarSign size={22} /> },
              { id: 'withdraw', name: 'Withdraw', icon: <ArrowUpFromLine size={22} /> },
              { id: 'ranking', name: 'Ranking', icon: <div className="relative"><Trophy size={22} /><div className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-white text-black text-[9px] font-black rounded-full flex items-center justify-center border-2 border-[#1a1a1a]">1</div></div> },
              ...
              { id: 'notifications', name: 'Notifications', icon: <div className="relative"><Bell size={22} /><div className="absolute top-0 right-0 w-1.5 h-1.5 bg-white rounded-full"></div></div> },
            ].map((nav, i) => (
              <button 
                key={i} 
              ...
              >
                ...
                <span className="text-[9px] font-black text-gray-500 uppercase tracking-widest text-center">{nav.name}</span>
              </button>
            ))}
          </div>
        </div>
          ...
          <div className="bg-[#e2f9cc] rounded-full p-4 flex items-center justify-between shadow-lg">
            <div className="flex items-center gap-5">
              <div className="w-14 h-14 bg-black/5 rounded-full flex items-center justify-center p-3 relative overflow-hidden">
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-20"><Wallet size={24} /></div>
                <img src="https://cdn-icons-png.flaticon.com/512/2488/2488749.png" className="w-full h-full object-contain relative z-10" alt="" />
              </div>
              <div className="flex flex-col">
                <span className="text-2xl font-black text-black leading-none">${availableBalance}</span>
                <span className="text-[10px] font-black text-black/60 uppercase tracking-widest mt-1">Total Deposit</span>
              </div>
            </div>
          </div>
          <div className="bg-[#d9e8fb] rounded-full p-4 flex items-center justify-between shadow-lg">
            <div className="flex items-center gap-5">
              <div className="w-14 h-14 bg-black/5 rounded-full flex items-center justify-center p-4">
                <div className="w-full h-full rounded-full bg-gray-400/50 flex items-center justify-center text-black/60 font-black text-xs">$</div>
              </div>
              <div className="flex flex-col">
                <span className="text-2xl font-black text-black leading-none">$0</span>
                <span className="text-[10px] font-black text-black/60 uppercase tracking-widest mt-1">Total Investment</span>
              </div>
            </div>
          </div>
        </div>
        <div className="flex justify-center pt-2 pb-12">
          <button className="bg-[#242424] text-[#888888] px-8 py-3 rounded-full text-[10px] font-black uppercase tracking-[0.2em] shadow-xl">Load more</button>
        </div>
      </div>

      <div className="hidden lg:grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pb-12">
        {stats.map((stat, idx) => (
          <div key={idx} className={`${stat.color} rounded-3xl p-6 space-y-4 border border-black/5 flex flex-col justify-between h-40 shadow-xl shadow-black/5 hover:-translate-y-1 transition duration-300 cursor-default group`}>
            <div className="flex justify-between items-start"><span className={`${stat.textColor} font-black text-2xl tracking-tighter`}>{stat.value}</span><div className="w-10 h-10 rounded-2xl bg-black/5 flex items-center justify-center text-black/40 group-hover:scale-110 transition">{stat.icon}</div></div>
            <span className={`${stat.textColor} text-[10px] font-black uppercase tracking-[0.2em] opacity-60`}>{stat.label}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  const renderRanking = () => (
    <div className="space-y-10 animate-in fade-in max-w-7xl mx-auto pb-32 px-4 md:px-0">
      <div className="flex items-center gap-4">
         <button onClick={() => handleTabChange('dashboard')} className="w-10 h-10 rounded-2xl bg-white/5 flex items-center justify-center text-gray-400 hover:bg-white/10 transition border border-white/5">
            <ChevronLeft size={20} />
         </button>
  ...
  const renderSchemaPreview = () => {
    ...
    return (
      <div className="space-y-10 animate-in fade-in duration-500 max-w-7xl mx-auto pb-32 px-4 md:px-0">
        <div className="flex items-center gap-4">
           <button onClick={() => handleTabChange('all-schemas')} className="w-10 h-10 rounded-2xl bg-white/5 flex items-center justify-center text-gray-400 hover:bg-white/10 transition border border-white/5">
              <ChevronLeft size={20} />
           </button>
              ...
              <table className="w-full text-left min-w-[500px]">
                 <tbody>
                    <tr className="border-b border-white/5">
                       <td className="py-5 px-6 text-[11px] font-black text-gray-500 uppercase tracking-widest border-r border-white/5 w-1/3">Select Schema:</td>
                       <td className="py-5 px-6">
                          ...
                          >
                             {INVESTMENT_PLANS.map(p => (
                               <option key={p.id} value={p.id} className="bg-[#1a1814]">{p.name}</option>
                             ))}
                          </select>
                       </td>
                    </tr>
                    <tr className="border-b border-white/5">
                       <td className="py-5 px-6 text-[11px] font-black text-gray-500 uppercase tracking-widest border-r border-white/5">Profit Holiday:</td>
                       <td className="py-5 px-6 text-sm font-bold text-gray-300">No</td>
                    </tr>
                    <tr className="border-b border-white/5">
                       <td className="py-5 px-6 text-[11px] font-black text-gray-500 uppercase tracking-widest border-r border-white/5">Amount:</td>
                       <td className="py-5 px-6 text-sm font-black text-gray-300">Minimum {selectedPlan.minInvestment} USD - Maximum {selectedPlan.maxInvestment} USD</td>
                    </tr>
                    <tr className="border-b border-white/5">
                       <td className="py-5 px-6 text-[11px] font-black text-gray-500 uppercase tracking-widest border-r border-white/5">Enter Amount:</td>
                       <td className="py-5 px-6">
                          <div className="relative max-w-[400px]">
                             ...
                             {isAmountTooLow && (
                               <div className="text-[10px] text-red-500 font-bold uppercase tracking-widest mt-1">Minimum {selectedPlan.minInvestment} USD Required</div>
                             )}
                          </div>
                       </td>
                    </tr>
                    <tr className="border-b border-white/5">
                       <td className="py-5 px-6 text-[11px] font-black text-gray-500 uppercase tracking-widest border-r border-white/5">Return of Interest:</td>
                       <td className="py-5 px-6 text-sm font-bold text-gray-300">{selectedPlan.returnLabel} ({selectedPlan.returnType})</td>
                    </tr>
                    <tr className="border-b border-white/5">
                       <td className="py-5 px-6 text-[11px] font-black text-gray-500 uppercase tracking-widest border-r border-white/5">Number of Period:</td>
                       <td className="py-5 px-6 text-sm font-bold text-gray-300">{selectedPlan.periods} Times</td>
                    </tr>
                    <tr>
                       <td className="py-5 px-6 text-[11px] font-black text-gray-500 uppercase tracking-widest border-r border-white/5">Capital Back:</td>
                       <td className="py-5 px-6 text-sm font-bold text-gray-300">{selectedPlan.capitalBack ? 'Yes' : 'No'}</td>
                    </tr>
                 </tbody>
              </table>
           </div>
  ...
  const renderTransactions = () => (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-7xl mx-auto pb-28">
      <div className="glass-card bg-[#1a1a1a]/40 border border-white/5 rounded-[2.5rem] p-6 md:p-12 shadow-2xl relative overflow-hidden">
        ...
        <h3 className="text-xl md:text-2xl font-black text-white uppercase tracking-tight mb-8">My Deposits / Transactions</h3>
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left min-w-[800px]">
            <thead>
              <tr className="bg-white/5 rounded-2xl overflow-hidden">
                <th className="py-4 px-6 text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 first:rounded-l-2xl">Description</th>
                <th className="py-4 px-6 text-[10px] font-black uppercase tracking-[0.2em] text-gray-500">Transaction ID</th>
                ...
                <th className="py-4 px-6 text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 last:rounded-r-2xl">Gateway</th>
              </tr>
            </thead>
            <tbody>
              <tr><td colSpan={7} className="py-24 text-center text-gray-600 text-xs font-bold uppercase tracking-widest">No transactions found.</td></tr>
            </tbody>
          </table>
        </div>
      </div>
      ...
      return (
        <div className="space-y-6 animate-in fade-in zoom-in duration-500 max-w-[800px] mx-auto pb-32 px-2 md:px-0 flex flex-col items-center">
           <div className="flex items-center gap-4 w-full">
             <button onClick={() => setDepositStep('entry')} className="w-10 h-10 md:w-12 md:h-12 rounded-2xl bg-white/5 flex items-center justify-center text-gray-400 hover:bg-white/10 transition border border-white/5 flex-shrink-0">
                <ChevronLeft size={20} />
             </button>
             <div className="space-y-0.5">
                <h3 className="text-xl md:text-2xl font-black text-white uppercase tracking-tight">Complete Payment</h3>
                <p className="text-[9px] md:text-[10px] font-bold text-gray-500 uppercase tracking-widest">Scan or copy address to pay</p>
             </div>
           </div>
           ...
           <div className="glass-card bg-[#141414]/90 border border-white/10 rounded-[2.5rem] p-6 md:p-12 text-center space-y-8 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5)] relative overflow-hidden w-full max-w-md md:max-w-xl">
            <div className="space-y-1">
              <span className="text-[9px] md:text-[10px] font-black text-amber-500 uppercase tracking-[0.3em]">Send Exactly</span>
              <div className="text-2xl md:text-4xl lg:text-5xl font-black text-white tracking-tight break-all px-2">{cryptoAmount}</div>
              <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest block">In {selectedToken.split(' ')[0]} (≈ ${depositAmount} USD)</span>
            </div>
            ...
            <div className="space-y-3 max-w-sm mx-auto w-full">
              <label className="text-[9px] font-black text-gray-500 uppercase tracking-widest block">Official Wallet Address</label>
              <div className="flex items-center gap-2 bg-black/60 rounded-xl md:rounded-2xl px-3 py-3 md:px-5 md:py-4 border border-white/10 group">
                <span className="text-[9px] md:text-[11px] lg:text-xs font-bold text-gray-400 truncate flex-grow text-left select-all">{address}</span>
                <button 
            ...
            <div className="pt-2 w-full">
              <button onClick={() => handleTabChange('transactions')} className="w-full max-w-sm mx-auto py-4 rounded-full bg-white/5 border border-white/10 text-white font-black uppercase text-[10px] tracking-widest hover:bg-white/10 transition active:scale-[0.98]">
                I Have Made Payment
              </button>
            </div>
          </div>
    ...
    return (
      <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-4xl mx-auto pb-28 px-2">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
           ...
           <button onClick={() => handleTabChange('transactions')} className="w-full md:w-auto px-8 py-4 rounded-2xl bg-gradient-to-r from-[#ff8c00] to-[#ff4500] text-white font-black uppercase text-[10px] tracking-widest shadow-xl shadow-orange-500/20 hover:scale-[1.02] transition">Deposit History</button>
        </div>
        ...
        <div className="glass-card bg-[#141414]/60 border border-white/5 rounded-[2.5rem] p-6 md:p-14 space-y-12 shadow-2xl relative overflow-hidden">
          <div className="space-y-4">
             <label className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] block">Payment Method</label>
             <div className="relative group">
                <select value={selectedToken} onChange={(e) => setSelectedToken(e.target.value as any)} className="w-full bg-[#1c1c1c] border border-white/5 rounded-2xl py-5 px-6 text-sm text-gray-300 focus:outline-none appearance-none cursor-pointer shadow-inner">
                   <option value="">--Select Token--</option>
                   <option value="BTC">Bitcoin (BTC)</option>
                   <option value="ETH (ERC20)">Ethereum (ETH ERC20)</option>
                   <option value="USDT (TRC20)">Tether (USDT TRC20)</option>
                </select>
                <div className="absolute right-6 top-1/2 -translate-y-1/2 pointer-events-none text-gray-500"><ChevronDown size={20} /></div>
             </div>
          </div>
          <div className="space-y-4">
             <label className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] block">Enter Amount (USD):</label>
             <div className="relative group">
                <input type="number" placeholder="0.00" value={depositAmount} onChange={(e) => setDepositAmount(e.target.value)} className="w-full bg-[#1c1c1c] border border-white/5 rounded-2xl py-6 px-8 text-xl md:text-2xl font-bold text-white focus:outline-none transition shadow-inner placeholder:text-gray-700" />
                <div className="absolute right-8 top-1/2 -translate-y-1/2 text-gray-500 bg-white/5 w-12 h-12 rounded-xl flex items-center justify-center border border-white/5 font-black text-lg">$</div>
             </div>
          </div>
          <div className="pt-6 flex justify-center md:justify-end">
             <button disabled={!selectedToken || !depositAmount || isPricesLoading} onClick={() => setDepositStep('payment')} className="w-full md:w-auto flex items-center justify-center gap-3 px-12 py-5 rounded-full bg-gradient-to-r from-[#ff8c00] to-[#ff4500] text-white font-black uppercase text-[11px] tracking-widest hover:scale-[1.02] transition transform active:scale-95 group disabled:opacity-50">Proceed to Payment</button>
          </div>
        </div>
      </div>
    );
      ...
      return (
        <div className="flex flex-col items-center justify-center min-h-[70vh] space-y-8 md:space-y-10 animate-in fade-in zoom-in duration-700 max-w-4xl mx-auto px-6">
           ...
           <div className="text-center space-y-4 max-w-lg mx-auto">
              <h3 className="text-2xl md:text-4xl font-black text-white uppercase tracking-tight leading-tight">Identity Verification Required</h3>
              <p className="text-xs md:text-sm font-medium text-gray-400 leading-relaxed uppercase tracking-wider">To ensure the security of your funds and comply with financial regulations, all withdrawals require <span className="text-amber-500">KYC Verification</span>.</p>
           </div>
           ...
           <div className="flex flex-col items-center gap-6 w-full max-w-sm">
              ...
              <button onClick={() => handleTabChange('dashboard')} className="text-[10px] font-black text-gray-600 uppercase tracking-widest hover:text-white transition">Return to Home</button>
           </div>
           ...
           <div className="pt-8 grid grid-cols-3 gap-4 md:gap-8 w-full">
              {[
                ...
                <div key={i} className="flex flex-col items-center gap-2">
                   <div className="w-8 h-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center">{item.icon}</div>
                   <span className="text-[8px] font-black text-gray-600 uppercase tracking-widest text-center">{item.label}</span>
                </div>
              ))}
           </div>
        </div>
    ...
    if (withdrawStep === 'address') {
      return (
        <div className="animate-in fade-in zoom-in duration-500 max-w-3xl mx-auto pb-32 px-2">
          <div className="glass-card bg-[#1a1a1a]/40 border border-white/5 rounded-[2.5rem] p-6 md:p-12 shadow-2xl relative overflow-hidden">
             <button onClick={() => setWithdrawStep('amount')} className="absolute top-6 left-6 md:top-8 md:left-8 w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-gray-400 hover:text-white transition"><ChevronLeft size={20} /></button>
             <h3 className="text-2xl md:text-3xl font-black text-white uppercase tracking-tight mb-10 text-center">Withdrawal Details</h3>
             
             <div className="space-y-10 max-w-md mx-auto">
                <div className="space-y-4">
                  <label className="text-[11px] font-black text-white/60 uppercase tracking-widest block">Select Token to Receive</label>
                  <div className="relative group">
                    <select 
                ...
                <div className="space-y-4">
                  <label className="text-[11px] font-black text-white/60 uppercase tracking-widest block">Recipient Wallet Address</label>
                  <input 
                    type="text" 
      ...
      <div className="animate-in fade-in duration-700 max-w-4xl mx-auto pb-32 px-2">
        <div className="glass-card bg-[#1a1a1a]/40 border border-white/5 rounded-[2.5rem] p-6 md:p-12 shadow-2xl relative overflow-hidden">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
            <h3 className="text-2xl md:text-3xl font-black text-white uppercase tracking-tight text-center md:text-left">Withdraw</h3>
          ...
          <div className="space-y-8 max-w-xl mx-auto lg:mx-0">
            <div className="space-y-4">
              <label className="text-[11px] font-black text-white/60 uppercase tracking-widest block">From Wallet</label>
              <div className="relative group">
            ...
            <div className="space-y-4">
              <label className="text-[11px] font-black text-white/60 uppercase tracking-widest block">Enter Amount</label>
              <div className="relative group">
                <input 
                  type="number" 
            ...
            <div className="space-y-4 pt-4">
              ...
              <div className="bg-[#1a1a1a]/60 border border-white/10 border-dashed rounded-[2rem] p-6 md:p-8 space-y-6">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-black text-white/60">${parseFloat(withdrawAmount || '0') + withdrawalCharge}</span>
                </div>
              </div>
            </div>
  ...
  const renderSettings = () => (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-7xl mx-auto pb-32 px-2 md:px-0">
      <div className="glass-card bg-[#141414]/60 border border-white/5 rounded-[2.5rem] p-6 md:p-12 shadow-2xl space-y-12 relative overflow-hidden">
        <h3 className="text-2xl font-black text-white uppercase tracking-tight text-center md:text-left">Profile Settings</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
           <button onClick={() => handleTabChange('kyc-verify')} className="flex items-center gap-4 md:gap-5 p-4 md:p-6 rounded-3xl bg-white/5 border border-white/10 text-left hover:bg-white/10 transition group">
              <div className="w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-amber-500/10 flex items-center justify-center text-amber-500 group-hover:scale-110 transition flex-shrink-0"><ShieldCheck size={28} /></div>
              <div className="space-y-0.5 flex-grow"><h4 className="text-[10px] md:text-[11px] font-black text-white uppercase tracking-widest">Verify Identity</h4><p className="text-[8px] md:text-[9px] text-gray-500 font-bold uppercase tracking-widest">Complete KYC verification</p></div>
              <ChevronRight className="text-gray-600 flex-shrink-0" size={18} />
           </button>
           <button onClick={() => handleTabChange('reset-password')} className="flex items-center gap-4 md:gap-5 p-4 md:p-6 rounded-3xl bg-white/5 border border-white/10 text-left hover:bg-white/10 transition group">
              <div className="w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-orange-500/10 flex items-center justify-center text-orange-500 group-hover:scale-110 transition flex-shrink-0"><Lock size={28} /></div>
              <div className="space-y-0.5 flex-grow"><h4 className="text-[10px] md:text-[11px] font-black text-white uppercase tracking-widest">Change Password</h4><p className="text-[8px] md:text-[9px] text-gray-500 font-bold uppercase tracking-widest">Update your security</p></div>
              <ChevronRight className="text-gray-600 flex-shrink-0" size={18} />
           </button>
        </div>
        ...
        <div className="space-y-4 pt-6 border-t border-white/5">
          ...
          <div onClick={() => fileInputRef.current?.click()} className="w-full max-w-[320px] aspect-[16/10] bg-[#1c1c1c] border border-white/5 border-dashed rounded-2xl flex flex-col items-center justify-center gap-3 group cursor-pointer hover:border-amber-500/30 transition-all shadow-inner relative overflow-hidden mx-auto md:mx-0">
            {avatarPreview ? <img src={avatarPreview} alt="Avatar" className="w-full h-full object-cover" /> : <><Camera size={32} className="text-gray-700 group-hover:text-amber-500/50" /><span className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500">Select Avatar</span></>}
            <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={(e) => handleImageChange(e, 'avatar')} />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {[
            ...
            { label: 'Email Address', name: 'email', type: 'email', placeholder: auth.currentUser?.email || '', readOnly: true },
            ...
            <div key={idx} className="space-y-2">
              <label className="text-[10px] md:text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] block">{field.label}</label>
              <div className="relative group">
                {field.type === 'select' ? (
                  <select name={field.name} disabled={field.disabled} value={profileData[field.name as keyof typeof profileData]} onChange={handleProfileChange} className={`w-full bg-[#1c1c1c] border border-white/5 rounded-2xl py-4 md:py-5 px-6 text-sm ${field.disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'} text-gray-300 focus:outline-none appearance-none shadow-inner`}>{field.options?.map(opt => <option key={opt} value={opt}>{opt}</option>)}</select>
                ) : (
                  <input name={field.name} type={field.type} value={field.readOnly ? field.placeholder : profileData[field.name as keyof typeof profileData]} onChange={handleProfileChange} readOnly={field.readOnly} className={`w-full bg-[#1c1c1c] border border-white/5 rounded-2xl py-4 md:py-5 px-6 text-sm ${field.readOnly ? 'text-gray-500 italic bg-black/20' : 'text-gray-300'} focus:outline-none transition shadow-inner`} />
                )}
                {field.type === 'select' && <div className="absolute right-6 top-1/2 -translate-y-1/2 pointer-events-none text-gray-500"><ChevronDown size={18} /></div>}
                {field.type === 'date' && <div className="absolute right-6 top-1/2 -translate-y-1/2 pointer-events-none text-gray-500"><Calendar size={18} /></div>}
              </div>
            </div>
          ))}
        </div>
        <div className="pt-6 flex justify-center md:justify-end"><button disabled={saveLoading} onClick={handleSaveProfile} className="w-full md:w-auto flex items-center justify-center gap-3 px-12 py-5 rounded-full bg-gradient-to-r from-[#ff8c00] to-[#ff4500] text-white font-black uppercase text-[11px] tracking-widest shadow-2xl hover:scale-[1.02] transition transform active:scale-95 group disabled:opacity-50">{saveLoading ? <Loader2 className="animate-spin" size={18} /> : <>Save Changes <ExternalLink size={18} /></>}</button></div>
      </div>
    </div>
  );
  ...
  const renderReferralContent = () => (
    <div className="space-y-8 animate-in fade-in max-w-2xl mx-auto pb-32 px-2 md:px-0">
      <h3 className="text-[11px] font-black text-white uppercase tracking-[0.25em] opacity-90 text-center md:text-left">REFERRAL AND TREE</h3>
      ...
      <div className="space-y-4">
        ...
        <p className="text-white/50 text-[9px] font-black uppercase tracking-widest text-center md:text-left">0 peoples are joined by using this URL</p>
      </div>
           ...
           <div className="w-full h-full rounded-[1.2rem] bg-gradient-to-br from-[#ff8c00] to-[#ff4500] p-[2px] shadow-2xl">
              <div className="w-full h-full bg-[#1c1c1c] rounded-[calc(1.2rem-2px)] flex flex-col items-center justify-center gap-2">
                 <div className="w-10 h-10 rounded-lg bg-white/5 flex items-center justify-center border border-white/10 p-1">
                    {avatarPreview ? <img src={avatarPreview} className="w-full h-full object-cover rounded-md" /> : <UserCircle2 size={24} className="text-amber-500 opacity-60" />}
                 </div>
      ...
      <div className="bg-[#0d0d0d]/60 border border-white/5 rounded-[2.5rem] p-6 md:p-8 shadow-2xl space-y-8">
        ...
        <div className="bg-[#141414] border border-white/5 border-dashed rounded-2xl h-24 flex items-center justify-center">
           <span className="text-[10px] font-bold text-gray-700 uppercase tracking-[0.2em] italic">NO DATA LOGS FOUND</span>
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-10 animate-in fade-in max-w-7xl mx-auto pb-32 px-2 md:px-0">
      <div className="glass-card bg-[#141414]/60 border border-white/5 rounded-[2.5rem] p-6 md:p-12 shadow-2xl space-y-10 relative overflow-hidden">
        <div className="space-y-2 text-center md:text-left">
           ...
           <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Secure your account by verifying your identity</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
          <div className="space-y-3">
            <label className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] block">Full Legal Name</label>
            <input type="text" placeholder="Enter full name" value={kycData.fullName} onChange={(e) => setKycData({...kycData, fullName: e.target.value})} className="w-full bg-[#1c1c1c] border border-white/5 rounded-2xl py-5 px-6 text-sm text-gray-300 focus:outline-none transition shadow-inner" />
          </div>
          <div className="space-y-3">
            <label className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] block">Date of Birth</label>
            <div className="relative">
              <input type="date" value={kycData.dob} onChange={(e) => setKycData({...kycData, dob: e.target.value})} className="w-full bg-[#1c1c1c] border border-white/5 rounded-2xl py-5 px-6 text-sm text-gray-300 focus:outline-none shadow-inner" />
              <Calendar size={18} className="absolute right-6 top-1/2 -translate-y-1/2 text-gray-500 pointer-events-none" />
            </div>
          </div>
          <div className="space-y-3">
            <label className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] block">Country</label>
            <div className="relative">
              <select value={kycData.country} onChange={(e) => setKycData({...kycData, country: e.target.value})} className="w-full bg-[#1c1c1c] border border-white/5 rounded-2xl py-5 px-6 text-sm text-gray-300 focus:outline-none appearance-none shadow-inner cursor-pointer">
          ...
          <div className="space-y-3">
            <label className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] block">Type of Document</label>
            <div className="relative">
              <select value={kycData.docType} onChange={(e) => setKycData({...kycData, docType: e.target.value})} className="w-full bg-[#1c1c1c] border border-white/5 rounded-2xl py-5 px-6 text-sm text-gray-300 focus:outline-none appearance-none shadow-inner cursor-pointer">
        ...
        <div className="space-y-6 pt-6 border-t border-white/5">
          <h4 className="text-[12px] md:text-[14px] font-black text-white uppercase tracking-widest text-center md:text-left">Document Upload</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              <div key={i} className="space-y-3">
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest block">{upload.label}</label>
                <div onClick={() => upload.ref.current?.click()} className="aspect-video bg-[#1c1c1c] border border-white/5 border-dashed rounded-3xl flex flex-col items-center justify-center gap-3 cursor-pointer hover:border-amber-500/30 transition-all shadow-inner relative overflow-hidden group">
                   {upload.state ? (
                     <img src={upload.state} className="w-full h-full object-cover" alt={upload.label} />
                   ) : (
                   ...
                   <input ref={upload.ref} type="file" className="hidden" accept="image/*" onChange={(e) => handleImageChange(e, upload.setter as any)} />
                </div>
              </div>
            ))}
          </div>
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'all-schemas': return <div className="space-y-10 animate-in fade-in max-w-7xl mx-auto pb-28 px-2 md:px-0"><div className="text-center md:text-left space-y-1"><h3 className="text-2xl md:text-4xl font-black text-white uppercase tracking-tight">Investment Plans</h3><p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Choose your financial growth strategy</p></div><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">{INVESTMENT_PLANS.map(plan => (<SchemaCard key={plan.id} plan={plan} onInvest={handleOpenPreview} />))}</div></div>;
      case 'referral': return renderReferralContent();
      case 'add-money': return renderAddMoney();
      case 'transactions':
      case 'logs': return renderTransactions();
      case 'settings': return renderSettings();
      case 'withdraw': return renderWithdraw();
      case 'ranking': return renderRanking();
      case 'support': return <div className="flex flex-col items-center justify-center min-h-[70vh] space-y-8 animate-in fade-in max-w-md mx-auto px-4"><div className="w-20 h-20 md:w-24 md:h-24 rounded-[2rem] bg-amber-500/10 flex items-center justify-center text-amber-500 shadow-2xl rotate-12"><LifeBuoy size={48} /></div><div className="text-center space-y-4"><h3 className="text-2xl md:text-3xl font-black text-white uppercase tracking-tight">Need Assistance?</h3><p className="text-xs md:text-sm font-medium text-gray-400 leading-relaxed uppercase tracking-wider">Our experts are live on Telegram to help you.</p></div><div className="flex flex-col items-center gap-4 w-full"><a href={telegramUrl} target="_blank" rel="noopener noreferrer" className="w-full flex items-center justify-center gap-3 py-5 rounded-full bg-[#0088cc] text-white font-black uppercase text-[11px] tracking-widest shadow-2xl hover:scale-[1.05] transition transform"><Send size={18} fill="white" /> Talk to Support</a><button onClick={() => handleTabChange('dashboard')} className="text-[10px] font-black text-gray-600 uppercase tracking-widest hover:text-white transition">Dashboard</button></div></div>;
      case 'kyc-verify': return renderKycVerify();
      case 'reset-password': return <div className="min-h-[80vh] flex flex-col items-center justify-center p-4 md:p-6 animate-in fade-in zoom-in duration-500 relative"><div className="w-full max-w-[600px] lg:max-w-[700px] glass-card p-8 md:p-16 rounded-[2.5rem] border border-white/10 z-10 text-center shadow-2xl relative overflow-hidden"><h1 className="text-2xl md:text-4xl font-black mb-3 uppercase text-white tracking-tight">Security Center</h1><p className="text-gray-400 text-[10px] font-bold uppercase tracking-[0.2em] mb-12">Update your Password</p>{resetSuccess ? (<div className="space-y-6"><div className="p-5 bg-green-500/10 border border-green-500/20 rounded-2xl text-green-400 text-xs font-bold uppercase tracking-widest">Success! Check your email.</div><button onClick={() => { setResetSuccess(false); handleTabChange('settings'); }} className="px-12 py-4 rounded-full bg-white/5 text-white font-black uppercase text-[10px] border border-white/10 transition">Settings</button></div>) : (<form onSubmit={handleResetPassword} className="space-y-8 text-left max-w-sm mx-auto"><div className="space-y-2"><input required type="email" value={resetEmail} onChange={(e) => setResetEmail(e.target.value)} placeholder="Email Address" className="w-full bg-[#1c1c1c]/60 border border-white/10 rounded-2xl py-5 px-6 text-sm text-white focus:outline-none transition-all shadow-inner" /></div><div className="pt-2 flex flex-col items-center"><button disabled={resetLoading} type="submit" className="w-full py-5 rounded-full bg-gradient-to-r from-[#ff8c00] to-[#ff4500] text-white font-black uppercase text-[11px] tracking-widest shadow-2xl hover:scale-105 transition transform active:scale-95 flex items-center justify-center gap-3">{resetLoading ? <Loader2 className="animate-spin" size={16} /> : "Reset Link"}</button></div></form>)}</div></div>;
      case 'schema-preview': return renderSchemaPreview();
    ...
    <div className="flex h-screen bg-[#0a0a0a] text-white font-display overflow-hidden selection:bg-amber-500/30">
        ...
        <div className="fixed top-24 right-4 md:right-8 lg:right-12 z-[200] w-[calc(100%-2rem)] max-w-[420px] animate-in slide-in-from-right duration-500">
          <div className="bg-white rounded-2xl shadow-2xl overflow-hidden relative border border-black/5">
            <div className="p-5 md:p-6 flex items-start gap-4">
               <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-amber-500/10 flex items-center justify-center text-amber-500 flex-shrink-0"><AlertTriangle size={20} className="md:size-[24px]" /></div>
               <div className="flex-grow space-y-1 pt-0.5">
                  ...
                  <p className="text-[13px] md:text-[14px] text-gray-500 font-bold leading-tight">Your account is unverified with KYC.</p>
               </div>
               <button onClick={() => setKycWarningVisible(false)} className="text-gray-400 hover:text-gray-600 p-1"><X size={18} /></button>
            </div>
            <div className="absolute bottom-0 left-0 h-[4px] bg-amber-500/10 w-full"><div className="h-full bg-gradient-to-r from-amber-400 to-amber-600 animate-kyc-timer"></div></div>
          </div>
        </div>
      )}
      ...
      <aside className={`bg-[#0d0d0d] border-r border-white/5 hidden lg:flex flex-col transition-all duration-300 z-50 ${sidebarOpen ? 'w-64' : 'w-24'}`}>
        <div className="p-6 flex items-center justify-between h-24">
          <img src="https://upload.wikimedia.org/wikipedia/commons/e/e8/Tesla_logo.png" alt="Tesla" className={`h-6 tesla-red-filter transition-all ${!sidebarOpen ? 'opacity-0 scale-0 w-0' : 'opacity-100 scale-100'}`} />
        ...
        <nav className="flex-grow px-4 py-4 space-y-2 overflow-y-auto custom-scrollbar">
          {menuItems.map((item, idx) => {
            ...
            const isActive = activeTab === item.id || (isSettingsSub && item.id === 'settings') || (isPlansSub && item.id === 'all-schemas');

            return (
              <button key={idx} onClick={() => handleTabChange(item.id)} className={`w-full flex items-center gap-4 px-4 py-4 rounded-[1.2rem] transition-all relative group ${isActive ? 'bg-gradient-to-r from-orange-500 to-red-600 text-white shadow-2xl shadow-orange-500/30' : 'text-gray-500 hover:bg-white/5 hover:text-white'}`}>
                <span className="flex-shrink-0">{item.icon}</span>
                {sidebarOpen && <span className="text-[10px] font-black uppercase tracking-[0.2em] truncate">{item.name}</span>}
              </button>
            );
          })}
        </nav>
        <div className="p-4 border-t border-white/5"><button onClick={onLogout} className="w-full flex items-center gap-4 px-4 py-4 rounded-2xl bg-white/5 text-gray-500 hover:bg-red-500/10 hover:text-red-500 transition-all group"><LogOut size={18} />{sidebarOpen && <span className="text-[10px] font-black uppercase tracking-widest">Logout</span>}</button></div>
      </aside>
      ...
      <div className="flex-grow flex flex-col h-full relative w-full overflow-hidden">
        <header className="h-20 lg:h-24 border-b border-white/5 px-4 md:px-8 flex items-center justify-between bg-[#0d0d0d]/80 backdrop-blur-3xl relative z-40 w-full">
          <div className="flex items-center gap-4">
            {isSubPage && (
              <button 
                onClick={() => handleTabChange(activeTab === 'schema-preview' ? 'all-schemas' : (activeTab === 'ranking' ? 'dashboard' : 'settings'))} 
            ...
            <div><h2 className="text-[11px] md:text-[13px] font-black uppercase tracking-[0.2em] text-white truncate max-w-[150px] md:max-w-none">{activeTab.replace('-', ' ')}</h2></div>
          </div>
          <div className="flex items-center gap-3 md:gap-6 relative">
            ...
            <div className="relative" ref={notificationRef}>
              ...
              {notificationOpen && (
                <div className="absolute top-[calc(100%+12px)] right-0 w-80 md:w-96 bg-[#1a1a1a] border border-white/10 rounded-[1.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.6)] p-6 z-50 animate-in fade-in slide-in-from-top-2 duration-300 overflow-hidden">
                   <h3 className="text-sm md:text-base font-black text-white uppercase tracking-widest mb-4">Notifications</h3>
                   ...
                   <div className="bg-[#242424]/60 border border-white/5 rounded-2xl p-8 flex flex-col items-center justify-center text-center">
                      <span className="text-[10px] md:text-[11px] font-bold text-gray-400 uppercase tracking-widest">No Notification Found!</span>
                   </div>
                </div>
              )}
            </div>
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              >
                {avatarPreview ? <img src={avatarPreview} alt="Profile" className="w-full h-full object-cover" /> : <User size={20} className="text-white brightness-0 invert" />}
              </button>
              ...
              {profileDropdownOpen && (
                <div className="absolute top-[calc(100%+12px)] right-0 w-64 bg-[#1a1a1a] border border-white/10 rounded-[1.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.5)] py-4 px-3 z-50 animate-in fade-in slide-in-from-top-2 duration-300">
                   <div className="space-y-1">
                      <button onClick={() => handleTabChange('settings')} className="w-full flex items-center gap-4 px-4 py-4 rounded-xl text-gray-300 hover:bg-white/5 hover:text-white transition-all group">
                      ...
                      <button onClick={() => handleTabChange('reset-password')} className="w-full flex items-center gap-4 px-4 py-4 rounded-xl text-gray-300 hover:bg-white/5 hover:text-white transition-all group">
                      ...
                      <button onClick={() => handleTabChange('support')} className="w-full flex items-center gap-4 px-4 py-4 rounded-xl text-gray-300 hover:bg-white/5 hover:text-white transition-all group">
                   ...
                   <div className="mt-3 pt-3 border-t border-white/5">
                      <button onClick={onLogout} className="w-full flex items-center gap-4 px-4 py-4 rounded-xl bg-red-500/10 text-red-500 hover:bg-red-500/20 transition-all group">
                        <div className="w-6 h-6 rounded bg-red-500/20 flex items-center justify-center"><LogOut size={14} className="ml-0.5" /></div>
        ...
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 h-24 bg-[#0d0d0d]/95 backdrop-blur-xl border-t border-white/10 px-4 flex items-center justify-around z-[100] shadow-[0_-8px_30px_rgb(0,0,0,0.5)]">
          {bottomNavItems.map((item) => {
            const isSettingsSub = ['kyc-verify', 'reset-password'].includes(activeTab);
            const isPlansSub = activeTab === 'schema-preview';
            const isActive = activeTab === item.id || (isSettingsSub && item.id === 'settings') || (isPlansSub && item.id === 'all-schemas') || (activeTab === 'ranking' && item.id === 'dashboard');

            return (
              <button key={item.id} onClick={() => handleTabChange(item.id as TabType)} className="flex flex-col items-center justify-center gap-1.5 min-w-[60px] transition-all">
                <div className={`relative flex items-center justify-center w-11 h-11 md:w-12 md:h-12 rounded-[16px] md:rounded-[18px] transition-all duration-300 ${isActive ? 'bg-gradient-to-b from-[#ff8c00] to-[#ff4500]' : 'text-gray-300'}`}>{React.cloneElement(item.icon as React.ReactElement<any>, { size: 22, className: isActive ? 'text-white' : 'text-white/60' })}</div>
                <span className={`text-[8px] md:text-[9px] font-black uppercase tracking-widest text-center ${isActive ? 'text-white' : 'text-white/50'}`}>{item.label}</span>
              </button>
            );
          })}
        </nav>
